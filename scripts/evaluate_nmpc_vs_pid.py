...
Usage:
    # 1. 自动智能寻找 data/ 目录下最新的 NMPC_ESO, 纯 NMPC, PID 日志并对比：
    python3 evaluate_nmpc_vs_pid.py

    # 2. 手动指定文件（支持任意顺序、通配符，脚本将自动按文件名智能归类）：
    python3 evaluate_nmpc_vs_pid.py data/flight_log_NMPC_* data/flight_log_NMPC_ESO_* data/flight_log_PID_*
"""
...
def find_latest_log(data_dir, group_type):
    """
    智能精确查找对应类型的最新日志：
    - 'NMPC_ESO': 包含 'ESO' 的 NMPC 日志
    - 'NMPC': 仅包含 'NMPC' 且严格不含 'ESO' / 'PID' 的纯 NMPC 日志
    - 'PID': 包含 'PID' 的基线日志
    """
    ...
    for f in all_files:
        ...
        if group_type == "NMPC_ESO":
            if "ESO" in fname:
                candidates.append(f)
        elif group_type == "PID":
            if "PID" in fname:
                candidates.append(f)
        elif group_type == "NMPC":
            if "NMPC" in fname and "ESO" not in fname and "PID" not in fname:
                candidates.append(f)
...
def classify_input_files(file_args, data_dir):
    """
    针对用户在命令行传入的文件路径（支持任意顺序、通配符展开），自动智能识别并分流：
    - ESO 组文件
    - 纯 NMPC 组文件
    - PID 基线组文件
    """
    if not file_args:
        ...
        return eso_file, nmpc_file, pid_file
    ...
    for f in expanded_files:
        fname = os.path.basename(f)
        if "ESO" in fname:
            eso_candidates.append(f)
        elif "PID" in fname:
            pid_candidates.append(f)
        elif "NMPC" in fname:
            nmpc_candidates.append(f)
        else:
            unknowns.append(f)

    eso_file = max(eso_candidates, key=os.path.getctime) if eso_candidates else None
    nmpc_file = max(nmpc_candidates, key=os.path.getctime) if nmpc_candidates else None
    pid_file = max(pid_candidates, key=os.path.getctime) if pid_candidates else None

    # 如果用户传入的文件未包含标准命名前缀，按顺序填充剩余空缺
    for u in unknowns:
        if eso_file is None:
            eso_file = u
        elif nmpc_file is None:
            nmpc_file = u
        elif pid_file is None:
            pid_file = u

    # 如果某项依然为空，尝试从 data 目录自动补齐最新日志
    if eso_file is None:
        eso_file = find_latest_log(data_dir, "NMPC_ESO")
...
def main():
    ...
    # 智能解析与分类用户传入的文件参数或自动查找
    eso_file, nmpc_file, pid_file = classify_input_files(sys.argv[1:], data_dir)

    print("\n" + "="*80)
    print("  [File Classification Result]")
    print(f"  - NMPC + ESO Log : {eso_file if eso_file else 'NOT FOUND'}")
    print(f"  - Pure NMPC Log  : {nmpc_file if nmpc_file else 'NOT FOUND'}")
    print(f"  - PID Baseline   : {pid_file if pid_file else 'NOT FOUND'}")
    print("="*80 + "\n")
    ...
    if df_eso is None and df_nmpc is None and df_pid is None:
